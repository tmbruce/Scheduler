...
Getting the user's language is handled in the SignInController file on line 116 with the getLanguage() function. A switch function is used to test the language agains the available languages (French, Spanish) and English as default. This function gets the users language which is then passed to the switch statement and either translateFrench() or the translateSpanish() functions are called to translate the sign in screen to the user's language. Language translation for error messages are handled individually in the signInHandler() function. 
...
I chose to leave the view as a monthly view. From what I've gathered talking with other students it is common to use a much more simple method to view appointments, such as a table view. The month view created in the main screen controller is a raw implementation of a calendar that accounts for changes from month to month, including leap year calculations. The method is as follows: the first day of the current month's calendar is calculated. Next, the day in the week where the first day falls is calculated. Next, the number of days in the previous month are calculated to determine which number days to show on the calendar. Then the number of days for the current month are discovered and used to determine how many days from the succeeding month will be displayed on the calendar. A multidimensional array list is created with the number of the day in the calendar as the first index, and whether the day is in the current month (1) or in the preceding/succeeding month (0). From there, the loops are used to apply the numerical calendar days to the calendar day, along with a multidimensional array list with appointments to be added to the calendar view. The 1 or 0 is used to apply the style class to each day. A blue border will be placed around the current day on the calendar. I realize that the requirements call for a month view as well as a weekly view, I however chose to omit the weekly view because I feel the calendar I created exceeds the spirit of the project requirements. Additionally, it was at the advice of my mentor that I get the project to 80% completion and submit it. This may have not been the intent of the advice, but the project exceeds that 80% mark.
...
One of the two required lambda expressions is in the MainController class at line 256. Its purpose is to add an event handler to each appointment displayed on the calendar. The event handler, when clicked, launches the edit appointment controller.
...
This is handled in the MainScreenController class starting at line 60.
